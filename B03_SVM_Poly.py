            ...
            for LoopNum in range(1, 51):
                ...
                for sort_name in ['B_stat_abs', 'B_stat_cnt', 'RE_stat_cnt']:
                    ...
                    for NumFS in range(1, NS+1):
                        ...
                        # params
                        Cs = np.logspace(-8, 2, 11, base=2)
                        gammas = np.logspace(-2, 8, 11, base=2)
                        degs = [2, 3, 4]
                        param_grid = dict(C=Cs, gamma=gammas, degree=degs)
                        grid = GridSearchCV(svm.SVR(kernel='poly'), param_grid=param_grid, cv=5).fit(fs_train, label_train)
                        ...
                        model_svm_rkf = svm.SVR(kernel='poly', C=C, gamma=gamma, degree=deg).fit(fs_train, label_train)
                    ...
                    np.savetxt(directory + '\Z_R2_'+sort_name+'_Poly.txt', Score_R2, fmt='%f')
                    np.savetxt(directory + '\Z_y_true_'+sort_name+'_Poly.txt', Y_True, fmt='%f')
                    np.savetxt(directory + '\Z_y_Pred_' + sort_name + '_Poly.txt', Y_Pred, fmt='%f')
