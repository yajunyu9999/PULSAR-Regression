            ...
            for LoopNum in range(1, 51):
                ...
                for sort_name in ['B_stat_abs', 'B_stat_cnt', 'RE_stat_cnt']:
                    ...
                    for NumFS in range(1, NS+1):
                        ...
                        grid = GridSearchCV(svm.SVR(kernel='rbf'), param_grid=param_grid, cv=5).fit(fs_train, label_train)
                        ...
                        model_svm_rkf = svm.SVR(kernel='rbf', C=C, gamma=gamma).fit(fs_train, label_train)
                    ...
                    np.savetxt(directory + '\Z_R2_'+sort_name+'_RBF.txt', Score_R2, fmt='%f')
                    np.savetxt(directory + '\Z_y_true_'+sort_name+'_RBF.txt', Y_True, fmt='%f')
                    np.savetxt(directory + '\Z_y_Pred_' + sort_name + '_RBF.txt', Y_Pred, fmt='%f')
