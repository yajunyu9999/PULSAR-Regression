            ...
            for LoopNum in range(1, 51):
                ...
                for sort_name in ['B_stat_abs', 'B_stat_cnt', 'RE_stat_cnt']:
                    ...
                    for NumFS in range(1, NS+1):
                        ...
                        # gammas = np.logspace(-20, 0, 21, base=2)
                        param_grid = dict(C=Cs)
                        grid = GridSearchCV(svm.SVR(kernel='linear'), param_grid=param_grid, cv=5).fit(fs_train, label_train)
                    ...
                    np.savetxt(directory + '\Z_R2_'+sort_name+'_Linear.txt', Score_R2, fmt='%f')
                    np.savetxt(directory + '\Z_y_true_'+sort_name+'_Linear.txt', Y_True, fmt='%f')
                    np.savetxt(directory + '\Z_y_Pred_' + sort_name + '_Linear.txt', Y_Pred, fmt='%f')
